        ...
        <div class=" group mt-2 flex h-9 ">
          ...
          {:else}
            <div class="dbClickLabel w-full ">
              ...
              <input bind:value={defaultSize} type="number" class="absolute bg-transparent py-3 text-heading focus:outline-none" />
            </div>
          {/if}
        </div>
        <div class="mt-3 h-0.5 w-full bg-primary/50" />
...
<style lang="postcss" global>
  ...
  .inputValue {
    @apply w-full rounded-md bg-primary px-4 pl-16 text-sm text-white focus:bg-secondary focus:outline-none focus:ring-2 focus:ring-primaryBlue;
  }
  .fontSizeSelect {
  ...
  .dbClickLabel {
    @apply relative flex cursor-pointer items-center justify-center rounded-md bg-primary py-1 text-xs outline-none group-focus-within:bg-secondary group-focus-within:text-primaryBlue group-focus-within:ring-2 group-focus-within:ring-primaryBlue;
  }
  .label-division2 {
