  ...
  <div class=" left-0 top-14 flex h-auto min-w-[330px] max-w-[330px] flex-col border-b border-black bg-secondary pt-4 pb-4" in:fly={{ y: -200, duration: 1500 }} out:fly={{ y: -200, duration: 2000 }}>
    <div class="flex ">
      <div class="w-full flex-col items-center px-4">
        ...
        <!-- image upload -->
        {#if dargAndDrop}
          <div class="mt-3" in:fly={{ y: -200, duration: 500 }} out:fly={{ y: -200, duration: 500 }}>
            <form enctype="multipart/form-data" id="formImage" accept="image">
              <label class="hover:border-gray flex h-28 w-full cursor-pointer appearance-none justify-center rounded-md border border-dotted border-gray-300 bg-secondary px-4 transition focus:outline-none">
        ...
        <!-- image preview -->
        {#if imageUploaded}
          <div class="mt-3 items-center" in:fly={{ y: -200, duration: 500 }} out:fly={{ y: -200, duration: 500 }}>
        ...
        <div class=" mt-2 flex items-center justify-between gap-3 text-base">
          <div class="group relative  mx-auto flex w-2/5 items-center justify-between rounded-md bg-primary focus:outline-none">
            <div class="labelSpan text-gray-400">X</div>
...
<style lang="postcss" global>
  .labelSpan {
    @apply absolute rounded-md px-2 py-2 text-sm font-bold outline-none focus:ring-1 focus:ring-primaryBlue group-focus-within:text-primaryBlue;
  }
  .lockPositionBtn {
    @apply flex h-8 w-8 items-center justify-center rounded-md hover:ring-2 hover:ring-primaryBlue active:bg-primaryBlue;
  }
  .inputField {
    @apply w-full rounded-md bg-primary px-2 py-2 pl-8 text-sm text-white focus:bg-secondary focus:outline-none focus:ring-2 focus:ring-primaryBlue;
  }
  .inputValue {
    @apply w-full rounded-md bg-primary px-4 py-2 pl-16 text-sm text-white focus:bg-secondary focus:outline-none focus:ring-2 focus:ring-primaryBlue;
  }
</style>
