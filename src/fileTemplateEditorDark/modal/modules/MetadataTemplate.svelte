...
<div class="grid grid-cols-2">
  ...
  <div class="relative mx-auto w-2/3">
    <button on:click={Dropdown} type="button" class="relative w-full rounded-md bg-secondary py-2 px-4 text-left text-sm uppercase text-primaryGray focus:outline-none focus:ring-2 focus:ring-primaryBlue">
      {Documenttype}
      <span class="absolute top-0 right-0 flex h-full items-center">
        <DropDownIcon />
      </span>
    </button>
    <div class:hidden={!showSelectOptions} class="absolute mt-2 w-full space-y-4 rounded-md bg-secondary text-sm text-primaryGray ring-1 ring-primaryBlue">
      <div class="w-full">
