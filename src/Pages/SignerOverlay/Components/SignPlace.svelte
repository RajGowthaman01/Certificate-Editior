<script>
  ...
  import InputDownArrow from "../svg/InputDownArrow.svelte"
    ...
    <div class="w-full relative">
      ...
      {#if toggleSwitchWithText}
        <div transition:slide class="flex mt-3 items-center justify-start">
          <button on:click={toggleSwitch} class="w-9 mr-2 h-5 rounded-full p-1 border-2 border-lightGray flex {$pdfSigning.signPlacement.horizontal ? 'justify-start' : 'justify-end'} items-center">
            <div class="w-3 h-3 rounded-full bg-primary_blue" />
          </button>
          <span class="text-thin text-textGray mb-1">{$pdfSigning.signPlacement.horizontal ? "Horizontaly Lock" : "vertically Lock"}</span>
        </div>
      {/if}
    </div>
