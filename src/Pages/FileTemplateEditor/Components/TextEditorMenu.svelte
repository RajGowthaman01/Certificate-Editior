      ...
      <div transition:slide={{ duration: 500 }} class="space-y-2">
        <div class="flex gap-2">
          {#if input}
            <div on:click={dropDown} class="group relative w-full">
              <button
                type="button"
                class="flex h-10 w-full items-center justify-between rounded-md border-2 border-none  bg-lightGray px-3 text-sm font-bold text-textGray focus:ring-2 focus:ring-primary_blue font-{fontFamily} focus:border-2 focus:outline-none"
              >
                {fontFamily}
                <DropdownArrow />
              </button>
              ...
              <div
                class="{fontName
                  ? 'flex'
                  : 'hidden'} absolute z-10 mt-2 w-full rounded-md border border-primary_blue bg-darkGray font-bold text-textGray"
              >
                <div class="w-full">
                  {#each dropDowns as fontStyle}
                    <option
                      on:click={() => changeFont(fontStyle)}
                      class="option-class font-${fontStyle}"
                    >
                      {fontStyle}
                    </option>
                  {/each}
        ...
        <div class="flex h-10 gap-1">
          <div class="flex w-1/2 gap-1">
            <button
              on:click={() => (alignValue = "Left")}
              class="align-input group"
            >
              <div class="hidden group-hover:block">
                <Tooltip tip="Left Align" />
              </div>
              <AlignLeft {alignValue} />
            </button>
            <button
              on:click={() => (alignValue = "Center")}
              class="align-input group"
            >
              <div class="hidden group-hover:block">
                <Tooltip tip="Center Align" />
              </div>
              <AlignCenter {alignValue} />
            </button>
            <button
              on:click={() => (alignValue = "Right")}
              class="align-input group"
            >
              <div class="hidden group-hover:block">
                <Tooltip tip="Right Align" />
              </div>
          ...
          <div class="flex w-1/2 gap-1">
            ...
            <button
              on:click={() => (styleValue = "Bold")}
              class="align-input group ml-1"
            >
              <div class="hidden group-hover:block"><Tooltip tip="Bold" /></div>
              <Bold {styleValue} />
            </button>
            <button
              on:click={() => (styleValue = "Italic")}
              class="align-input group"
            >
              <div class="hidden group-hover:block">
                <Tooltip tip="Italic" />
        ...
        {#if inputBoxShow}
          <div class="grid h-10 grid-cols-6 gap-2">
            <button
              ...
              class="size group relative col-span-1 {disable
                ? 'cursor-not-allowed stroke-stone-600'
                : 'cursor-pointer stroke-textGray'} "
            >
              <div class="hidden group-hover:block">
                <Tooltip tip="Min Size" />
              </div>
              <SubNumber />
            </button>
            <div
              class="group col-span-4 flex items-center justify-center gap-2"
            >
              {#each number as numbers (numbers.id)}
                <button
                  on:dblclick={InputShow}
                  animate:flip={{ duration: 1000 }}
                  on:click={activeSize(numbers.value)}
                  class="size1 hover:text-primary_blue  group-active:text-darkGray  {sizeValue ==
                  numbers.value
                    ? 'text-white'
                    : 'text-gray-500'}">{numbers.value}</button
                >
              {/each}
            </div>
            <button
              on:click={increaseFontSize}
              class="size group relative col-span-1 -ml-1 {hide
                ? 'cursor-not-allowed stroke-stone-600'
                : 'cursor-pointer stroke-textGray'}"
            >
              <div class="hidden group-hover:block">
                <Tooltip tip="Max Size" />
              </div>
              <AddNumber />
            </button>
          </div>
        ...
        <div class="flex w-full items-center gap-4 pl-3">
          {#each color as colors}
            <button
              on:click={activeColor(colors)}
              class="dot  rounded-full p-0.5 {colorValue == colors
                ? 'ring-2 ring-white ring-offset-2 ring-offset-gray-700'
                : ''} focus:outline-none focus:ring-2 focus:ring-primary_blue"
              style="background:{colors}"
            />
          {/each}
          <label
            for="input-color"
            class="relative flex items-center justify-center fill-textGray pr-0.5 "
          >
            <button
              class="rounded-full focus:outline-none focus:ring-2 focus:ring-primary_blue"
            >
              <img
                ...
                src="https://img.favpng.com/7/23/16/color-wheel-rgb-color-model-color-gradient-complementary-colors-png-favpng-6qVjQ1RxkyK30g6yUQuZiLpQp.jpg"
                alt=""
              />
            </button>
            <input
    ...
    <div class="flex items-center justify-end gap-2 text-textGray">
      ...
      {:else}
        <button on:click={() => (minimize = true)} class="final-card group">
          ...
          <Maximize />
          <div
            class="absolute  top-0 right-0 h-2 w-2 {textName
              ? 'animate-ping'
