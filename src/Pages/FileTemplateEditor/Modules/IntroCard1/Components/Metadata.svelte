...
<div class="grid w-full grid-cols-2 gap-5">
  <div class="ml-20 space-y-10">
    <h1 class="text-4xl font-bold text-gray1 dark:text-white">
      Metadata Template
    </h1>
  ...
  <div class="group relative ml-20 flex w-full">
    <button
      on:click={() => {
        dropDown = !dropDown
      }}
    ...
    <div
      class="{dropDown
        ? 'flex flex-col'
        : 'hidden'} absolute top-14 z-10 w-3/4 rounded-md border-2 border-primary_blue font-bold text-textGray"
    >
...
<style lang="postcss">
  .option-class {
    @apply w-full rounded-md border-b-2 px-3 py-1.5 text-sm font-bold text-textGray hover:text-primary_blue;
  }
</style>
