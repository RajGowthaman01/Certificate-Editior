...
<div class:hidden={!modalOverLay} class="bg-topNavSvgBg/80 absolute flex h-screen w-screen items-center justify-center dark:bg-black/70 sm:p-5">
	...
	{/if}
	<div class="bg-heading shadow-box_Shadow dark:bg-certificateSection relative aspect-[15/7] w-[700px] rounded-md 2xl:aspect-[16/8] 2xl:w-[800px]" in:fade out:fade>
		<!-- {#if Close} -->
...
<style lang="postcss" global>
	.pagination {
		@apply hover:bg-primary_blue focus:bg-primary_blue dark:bg-primary h-4 w-4 rounded-full bg-gray-300 text-xs hover:text-white focus:outline-none active:scale-125 md:flex;
	}
</style>
