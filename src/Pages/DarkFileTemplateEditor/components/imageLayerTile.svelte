...
<div
  class:active
  class="flex justify-between border-b bg-lightGray px-4 dark:border-black dark:bg-primary ">
  ...
  <div class:hidden={editSection} class="flex items-center">
    <button
      type="button"
      class="group relative flex h-8 w-8 items-center justify-center rounded-md transition duration-500 ease-in-out hover:translate-y-1"
      on:click={editUpload}
      on:click={() => (textEditSection = false)}>
      <span>
