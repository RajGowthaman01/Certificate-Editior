...
<div class="relative flex h-screen w-screen flex-col overflow-hidden">
  <div
    class="bg-lightHeaderSection h-14 w-full items-center justify-center border-b-0 border-inherit py-2 dark:bg-secondary">
    <div class="justify-right flex">
      <div class="flex">
        <div
          class="left-0 top-0 grid min-w-[330px] max-w-[330px] grid-cols-3 justify-end ">
          <button class="Button">
            <Save /> Save
          </button>
          <button class="Button">
      ...
      <div
        class="flex w-[330px] flex-row items-center justify-between gap-8 border-l border-gray-700 px-8">
        <!-- <CardIconNav />
				<SettingIconNav />
				<StackedCircles />
				<ArrowPointer /> -->
        {#if darkMode}
          <button
            ...
            class="absolute top-1 right-5 rounded-lg bg-gray-500 py-1 px-2 capitalize text-white   focus:outline-none ">
            darkMode
          </button>
        {:else}
          <button
            on:click={toggleLightDark}
            class="absolute top-1 right-5 rounded-lg bg-white py-1 px-2 capitalize text-gray-800   focus:outline-none ">
            lightMode
          </button>
        {/if}
      </div>
